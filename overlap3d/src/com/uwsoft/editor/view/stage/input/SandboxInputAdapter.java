...
public class SandboxInputAdapter implements InputProcessor {
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		PerspectiveCamera camera = ComponentRetriever.get(rootEntity, CameraComponent.class).camera;

		/*
		if (screenX < Gdx.graphics.getWidth() || screenX >= viewPort.getScreenX() + viewPort.getScreenWidth()) return false;
...
//		}
	...
	public Vector2 screenToSceneCoordinates (Entity root, Vector2 screenCoords) {
		CameraComponent cameraComponent = ComponentRetriever.get(root, CameraComponent.class);
		//cameraComponent.viewPort.unproject(screenCoords);
		return screenCoords;
	}
