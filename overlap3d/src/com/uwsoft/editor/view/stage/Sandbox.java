...
import com.uwsoft.editor.renderer.components.CameraComponent;
...
//    }
    ...
    public void setZoomPercent(float percent) {
        ...
        getCamera().translate(tmpV1.set(getCamera().direction).scl((percent-zoomPercent)*20f));
        getCamera().update();
    ...
    public CameraComponent getViewportComponent() {
        if(getCurrentViewingEntity() == null) return null;
        CameraComponent cameraComponent = ComponentRetriever.get(getCurrentViewingEntity(), CameraComponent.class);
        return cameraComponent;
    }

    public PerspectiveCamera getCamera() {
        CameraComponent cameraComponent = getViewportComponent();
        if(cameraComponent == null) return null;
    ...
    public Vector2 screenToWorld(Vector2 vector) {
        ...
        vector.x = (vector.x - (Gdx.graphics.getWidth()/2 - getCamera().position.x*pixelPerWU/1f))*1f;
        vector.y = (vector.y - (Gdx.graphics.getHeight()/2 - getCamera().position.y*pixelPerWU/1f))*1f;
    ...
    public Vector2 worldToScreen(Vector2 vector) {
        ...
        int pixelPerWU = sceneControl.sceneLoader.getRm().getProjectVO().pixelToWorld;
        PerspectiveCamera camera = Sandbox.getInstance().getCamera();
        //Viewport viewport = Sandbox.getInstance().getViewport();
        vector.x = vector.x/1f + (camera.viewportWidth/2 - (camera.position.x)/1f);
        vector.y = vector.y/1f + (camera.viewportHeight/2 - (camera.position.y)/1f);
