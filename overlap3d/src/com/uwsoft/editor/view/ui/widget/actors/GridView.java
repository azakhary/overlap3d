...
public class GridView extends Actor {
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		PerspectiveCamera runtimeCamera = Sandbox.getInstance().getCamera();
		batch.setProjectionMatrix(uiCamera.projection);
		zeroLabel.draw(batch, parentAlpha);
		zeroLabel.setX(-(runtimeCamera.position.x*pixelsPerWU)/1f - 5 - zeroLabel.getWidth());
		zeroLabel.setY(-(runtimeCamera.position.y*pixelsPerWU)/1f- zeroLabel.getHeight());
	}
	...
	private void drawLines() {
		...
		PerspectiveCamera runtimeCamera = Sandbox.getInstance().getCamera();
		float offsetX = (runtimeCamera.position.x*pixelsPerWU)/1f % gridSize;
		float offsetY = (runtimeCamera.position.y*pixelsPerWU)/1f % gridSize;
	...
	private Color getLineColor(int i) {
		...
		float offsetTmp = ((runtimeCamera.position.x*pixelsPerWU)/1f) / gridSize;
		if(i >= gridLinesCount) {
			i-= gridLinesCount;
			offsetTmp = ((runtimeCamera.position.y*pixelsPerWU)/1f) / gridSize;
		}
